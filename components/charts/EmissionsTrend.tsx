    ...
    useEffect(() => {
        ...
        try {
            ...
            const grouped = entries.reduce((acc: any, entry) => {
                ...
                if (!acc[date]) {
                    // Используем текущую локаль i18n для форматирования даты
    ...
    return (
        <Card>
            ...
            <CardContent>
                ...
                <div className="mt-4 text-center text-sm text-muted-foreground flex flex-wrap justify-center gap-4">
                    <span className="inline-flex items-center">
                        <div className="w-4 h-0.5 bg-emerald-500 mr-1"></div>
                        <span className="text-xs">{t('overall_trend')}</span>
                    </span>
                    <span className="inline-flex items-center">
                        <div className="w-4 h-0.5 bg-blue-500 border-dashed mr-1" style={{ background: 'repeating-linear-gradient(90deg, #3b82f6, #3b82f6 3px, transparent 3px, transparent 8px)' }}></div>
                        <span className="text-xs">{t('your_entries')}</span>
                    </span>
                    <span className="inline-flex items-center">
                        <div className="w-4 h-0.5 bg-amber-500 border-dashed mr-1" style={{ background: 'repeating-linear-gradient(90deg, #f59e0b, #f59e0b 3px, transparent 3px, transparent 8px)' }}></div>
                        <span className="text-xs">{t('auto_generation')}</span>
                    </span>
                </div>
            </CardContent>
        </Card>
