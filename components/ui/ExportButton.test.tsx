...
describe('ExportButton', () => {
    ...
    it('отображается только если есть записи', () => {
    ...
    it('не отображается, если записей нет', () => {
        render(<ExportButton entries={[]} userProfile={mockUserProfile} />);
        expect(screen.queryByRole('button')).not.toBeInTheDocument();
    });

    it('открывает выпадающее меню при клике', async () => {
    ...
    it('экспортирует в CSV', async () => {
        ...
        expect(mockExport).toHaveBeenCalledWith(mockEntries, 'csv', mockUserProfile);
        expect(global.URL.createObjectURL).toHaveBeenCalled();
    });

    it('экспортирует в JSON', async () => {
        const user = userEvent.setup();
        ...
        expect(mockExport).toHaveBeenCalledWith(mockEntries, 'json', mockUserProfile);
    });

    it('экспортирует в HTML', async () => {
        const user = userEvent.setup();
        ...
        expect(mockExport).toHaveBeenCalledWith(mockEntries, 'html', mockUserProfile);
    });

    it('экспортирует в PDF', async () => {
        const user = userEvent.setup();
    ...
    it('показывает спиннер при загрузке', async () => {
        ...
        // ✅ Находим основную кнопку по aria-label
        ...
        // ✅ Проверяем, что основная кнопка отключена
        expect(mainButton).toBeDisabled();

        // ✅ Проверяем, что спиннер виден внутри основной кнопки
        expect(screen.getByText('Loader2')).toBeInTheDocument();
    });

    it('обрабатывает ошибку экспорта', async () => {
        const user = userEvent.setup();
