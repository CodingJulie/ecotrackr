...
export default function ExportButton({ entries, userProfile }: ExportButtonProps) {
    ...
    const handleExport = async (format: 'csv' | 'json' | 'html' | 'pdf') => {
        ...
        try {
            ...
            link.download = `ecotrackr-data-${new Date().toISOString().split('T')[0]}.${format === 'html' ? 'html' : format}`;
    ...
    const generatePDF = async () => {
        try {
            ...
            const avgCO2 = entries.length > 0 ? totalCO2 / entries.length : 0;

            // Форматирование даты с учётом текущего языка
    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 shadow-lg rounded-lg">
                <DropdownMenuItem onClick={() => handleExport('pdf')} aria-label={t('export_pdf_aria')}>
                    <File className="mr-2 h-4 w-4 text-red-500" /> {t('export_pdf')}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleExport('csv')} aria-label={t('export_csv_aria')}>
                    <FileSpreadsheet className="mr-2 h-4 w-4 text-green-500" /> {t('export_csv')}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleExport('json')} aria-label={t('export_json_aria')}>
                    <FileJson className="mr-2 h-4 w-4 text-blue-500" /> {t('export_json')}
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => handleExport('html')} aria-label={t('export_html_aria')}>
                    <FileText className="mr-2 h-4 w-4 text-purple-500" /> {t('export_html')}
                </DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
    );
