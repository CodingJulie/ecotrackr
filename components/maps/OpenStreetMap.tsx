...
const createCustomIcon = (isDark: boolean) => {
    ...
    return new L.DivIcon({
        ...
        html: `
      <svg width="42" height="48" viewBox="0 0 42 48" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Маркер на карте">
        <title>Маркер места</title>
...
export default function OpenStreetMap({ mapPoints }: OpenStreetMapProps) {
        ...
        const searchAddress = async () => {
            ...
            try {
                const response = await fetch(
                    `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(debouncedSearchQuery)}&format=json&limit=5&addressdetails=1&accept-language=ru`
                );
    ...
    useEffect(() => {
        if (mapRef.current) updateHeatmap();
    }, [updateHeatmap]);
            ...
            <div className="w-full relative pt-4">
                <div className="relative h-[520px] rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 shadow-lg">
                    ...
                    <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-40 w-96">
                        <div className="relative" ref={searchInputRef}>
                            <div className="relative">
                                ...
                                />
                                {isSearching && <Loader2 className="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 animate-spin text-muted-foreground" aria-hidden="true" />}
                            </div>
                    ...
                    <MapContainer
                        ...
                        ref={mapRef}
                        whenReady={() => setTimeout(() => updateHeatmap(), 100)}
                    >
                    ...
                    {/* Правая панель */}
                    <div className={`absolute top-4 right-4 z-40 flex flex-col gap-3 p-2 rounded-2xl ${panelBgClass} shadow-lg`}>
                        <div className="flex flex-col gap-1">
                            ...
                            >
                                <Layers className="mr-2 h-4 w-4" aria-hidden="true" /> {t('satellite')}
                            </Button>
                        </div>
                        <Button
                    ...
                    <div className={`absolute bottom-4 left-1/2 transform -translate-x-1/2 z-40 flex gap-2 p-2 rounded-full ${panelBgClass} shadow-lg`}>
                    ...
                    {showHeatSettings && (
                        <div className={`absolute bottom-24 right-4 z-40 w-80 rounded-2xl shadow-2xl border p-4 ${isDark ? 'bg-zinc-900 border-white/20' : 'bg-white border-gray-200'}`}>
                ...
                <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
                    <DialogContent className={`z-50 ${isDark ? 'bg-zinc-900 border-white/20' : 'bg-white border-gray-200'}`}>
                        <DialogHeader>
