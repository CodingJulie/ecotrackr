...
function calculatePeriodCO2(lifestyle: UserLifestyle, days: number) {
    ...
    const flightCO2 = lifestyle.weekly_flights * weeks * 500 * EMISSION_FACTORS.additional.flight;
    const plasticCO2 = lifestyle.weekly_plastic_use * weeks * EMISSION_FACTORS.additional.plastic;
    const recyclingSaving = lifestyle.recycling
        ? lifestyle.weekly_plastic_use * weeks * EMISSION_FACTORS.additional.recycling_saving
        : 0;
    ...
    return {
        ...
        food: foodCO2,
        home: homeCO2,
        additional: flightCO2 + plasticCO2 - recyclingSaving,
        total: housingCO2 + heatingCO2 + transportCO2 + foodCO2 + homeCO2 + flightCO2 + plasticCO2 - recyclingSaving,
    };
}
...
function buildEntriesForBatch(
    ...
    if (lifestyle.weekly_flights > 0) {
        entries.push({
            ...
            co2e: Math.round(lifestyle.weekly_flights * weeks * 500 * EMISSION_FACTORS.additional.flight),
            date: dateStr,
        });
    }
...
function NumberInput({ value, onChange, min = 0, max = 999, placeholder, className }: any) {
    const [localValue, setLocalValue] = useState(value.toString());
    useEffect(() => setLocalValue(value.toString()), [value]);
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const val = e.target.value;
        if (val === '') { setLocalValue(''); onChange(0); return; }
    ...
    return (
        <Input
            ...
            className={`h-8 text-sm bg-white dark:bg-zinc-800 ${className || ''}`}
        />
    );
}
...
export default function AutoGenerationWidget({ entries, user, onGenerated }: AutoGenerationWidgetProps) {
    ...
    const generateEntries = async () => {
        ...
        try {
            ...
            const batchDays = generationPeriod.value === 'week' || generationPeriod.value === 'month'
                ...
                : 30;
            const numPeriods = generationPeriod.value === 'week' ? 1 :
                generationPeriod.value === 'month' ? 1 :
            ...
            await supabase
                ...
                .delete()
                .eq('user_id', activeUser.id)
                .in('activity', ['meat_heavy', 'mixed', 'vegetarian', 'utilities', 'plane']);
            ...
            if (insertError) {
                ({ data: inserted, error: insertError } = await supabase
                    .from('footprint_entries')
                    .insert(entriesToInsert.map((entry) => ({ ...entry, is_auto_generated: true })))
                    .select('id, co2e, date, category, activity, value, is_auto_generated'));
            }
            ...
            setGenerationMessage(t('generation_error_detail', {
                message: error?.message || t('generation_error'),
            }));
        } finally {
            setGenerating(false);
        }
    ...
    // Получаем переведённые названия периодов
    const getPeriodLabel = (value: string) => t(`period_${value}`);

    // Опции транспорта с переводами
        ...
        <Card className="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30">
            ...
            <CardContent className="space-y-4">
                <div className="flex items-center gap-4 p-3 bg-white/50 dark:bg-black/20 rounded-lg">
                    ...
                    <div className="flex-1">
                        ...
                        <Select
                            ...
                            onValueChange={(v: any) => setGenerationPeriod(GENERATION_PERIODS.find(p => p.value === v) || GENERATION_PERIODS[1])}
                        >
                ...
                {estimatedCO2 !== null && (
                    <div className="p-4 bg-emerald-100/50 dark:bg-emerald-900/30 rounded-xl text-center">
                        ...
                        <p className="text-xs text-muted-foreground">
                            {t('kg_co2_period', { period: getPeriodLabel(generationPeriod.value).toLowerCase() })}
                        </p>
                    </div>
                )}
                ...
                <Button
                    ...
                    aria-label={t('generate_for_period_aria', { period: getPeriodLabel(generationPeriod.value) })}
                >
                    <RefreshCw className={`mr-2 h-4 w-4 ${generating ? 'animate-spin' : ''}`} />
                    {generating ? t('generating') : t('generate_for_period', { period: getPeriodLabel(generationPeriod.value).toLowerCase() })}
                </Button>
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <Select value={lifestyle.housing_type} onValueChange={(v: any) => setLifestyle({...lifestyle, housing_type: v})}>
                                ...
                                <div>
                                    ...
                                    <Select value={lifestyle.location_type} onValueChange={(v: any) => setLifestyle({...lifestyle, location_type: v})}>
                                        <SelectTrigger className="h-8 text-sm bg-white dark:bg-zinc-800">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent className="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
                                ...
                                <div>
                                    ...
                                    <NumberInput value={lifestyle.house_area} onChange={(val: number) => setLifestyle({...lifestyle, house_area: val})} min={1} placeholder={t('area_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('residents')}</Label>
                                    <NumberInput value={lifestyle.residents_count} onChange={(val: number) => setLifestyle({...lifestyle, residents_count: val})} min={1} placeholder={t('residents_placeholder')} />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="col-span-2">
                                    ...
                                    <Select value={lifestyle.transport_primary} onValueChange={(v: any) => setLifestyle({...lifestyle, transport_primary: v})}>
                                        <SelectTrigger className="h-8 text-sm bg-white dark:bg-zinc-800">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent className="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
                                ...
                                <div>
                                    ...
                                    <NumberInput value={lifestyle.transport_days_per_week} onChange={(val: number) => setLifestyle({...lifestyle, transport_days_per_week: val})} min={0} max={7} placeholder={t('days_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('km_per_day')}</Label>
                                    <NumberInput value={lifestyle.transport_distance_km} onChange={(val: number) => setLifestyle({...lifestyle, transport_distance_km: val})} min={0} placeholder={t('km_placeholder')} />
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <NumberInput value={lifestyle.meals_per_day} onChange={(val: number) => setLifestyle({...lifestyle, meals_per_day: val})} min={0} max={10} placeholder={t('meals_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('meat_frequency')}</Label>
                                    <Select value={lifestyle.meat_frequency} onValueChange={(v: any) => setLifestyle({...lifestyle, meat_frequency: v})}>
                                        <SelectTrigger className="h-8 text-sm bg-white dark:bg-zinc-800">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent className="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <NumberInput value={lifestyle.showers_per_day} onChange={(val: number) => setLifestyle({...lifestyle, showers_per_day: val})} min={0} max={10} placeholder={t('showers_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('computer_hours')}</Label>
                                    <NumberInput value={lifestyle.computer_hours_per_day} onChange={(val: number) => setLifestyle({...lifestyle, computer_hours_per_day: val})} min={0} max={24} placeholder={t('hours_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('heating')}</Label>
                                    <Select value={lifestyle.heating_type} onValueChange={(v: any) => setLifestyle({...lifestyle, heating_type: v})}>
                                        <SelectTrigger className="h-8 text-sm bg-white dark:bg-zinc-800">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent className="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <NumberInput value={lifestyle.weekly_flights} onChange={(val: number) => setLifestyle({...lifestyle, weekly_flights: val})} min={0} placeholder={t('flights_placeholder')} />
                                </div>
                                <div>
                                    <Label className="text-xs">{t('plastic_kg')}</Label>
                                    <NumberInput value={lifestyle.weekly_plastic_use} onChange={(val: number) => setLifestyle({...lifestyle, weekly_plastic_use: val})} min={0} placeholder={t('plastic_placeholder')} />
                                </div>
                                <div className="col-span-2 flex items-center justify-between">
                                    <Label className="text-xs">{t('recycling')}</Label>
                                    <Switch
                                        checked={lifestyle.recycling}
                                        onCheckedChange={(v) => setLifestyle({...lifestyle, recycling: v})}
                                    />
                                </div>
                            </div>
