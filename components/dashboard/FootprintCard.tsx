...
interface FootprintCardProps {
  totalCO2: number;
  period: string;
  comparison?: number; // % по сравнению с прошлым месяцем
}
      ...
      <Card className="overflow-hidden border-emerald-100 dark:border-emerald-900">
        ...
        <CardContent>
          <div className="flex items-baseline gap-1">
            ...
            <span className="text-2xl text-muted-foreground">кг CO₂e</span>
          </div>

          {comparison && (
            <div className={`flex items-center gap-1 text-sm mt-2 ${comparison < 0 ? 'text-emerald-600' : 'text-amber-600'}`}>
              <TrendingDown className="w-4 h-4" />
              {comparison > 0 ? '+' : ''}{comparison}% к прошлому месяцу
            </div>
          )}

          <div className="mt-4 text-xs text-muted-foreground">
            {isGood
              ? "Отличный результат! Вы ниже среднего показателя."
