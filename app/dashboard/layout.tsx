...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                const name = profile?.name || session.user.email?.split('@')[0] || 'Пользователь';
    ...
    return (
        <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
            <header className="hidden md:block sticky top-0 z-50 border-b border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl">
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        <Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={isDark ? "Переключить на светлую тему" : "Переключить на тёмную тему"}>
            ...
            <div className="flex">
                <aside className="fixed left-0 top-0 bottom-0 z-50 w-16 border-r border-zinc-200 dark:border-zinc-800 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-xl py-3 px-2 flex md:hidden flex-col items-center">
                    <Link
                        href="/dashboard"
                        aria-label={t('app_name')}
                        className="flex items-center justify-center w-10 h-10 mb-4 rounded-2xl hover:opacity-80 transition-opacity"
                    >
                    ...
                    <div className="pt-3 mt-auto border-t border-zinc-200 dark:border-zinc-800 w-full flex flex-col items-center gap-2">
                        <Button variant="ghost" size="icon" onClick={toggleTheme} aria-label={isDark ? "Переключить на светлую тему" : "Переключить на тёмную тему"}>
                            {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                        </Button>
                        ...
                        <button
                            ...
                            title={t('sign_out')}
                            className="flex items-center justify-center w-10 h-10 rounded-2xl text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all"
                        >
