...
**Open-source PWA для оценки личного CO₂e**
...
**EcoTrackr** — личное веб-приложение: можно записывать повседневные активности (транспорт, питание, энергия, покупки) и смотреть оценку CO₂e на дашборде. Это pet-проект, не продукт с аудиторией.

### Для кого

- Попробовать стек Next.js + PWA + Supabase + Web Workers
- Посмотреть, как устроен калькулятор CO₂e и дашборд
- Форк и доработка — репозиторий открытый
...
| **AI Insights** | Подсказки через Google Gemini, если задан API-ключ; иначе fallback |
