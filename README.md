...
[Демо](https://ecotrackr.com) · [Быстрый старт](#-быстрый-старт) · [API](#-api) · [Лицензия](#-лицензия)
...
<table>
  ...
  <tr>
    <td>
      <picture>
        <source media="(prefers-color-scheme: dark)" srcset="public/screenshot-desktop-dark.png" />
        <source media="(prefers-color-scheme: light)" srcset="public/screenshot-desktop-light.png" />
        <img src="public/screenshot-desktop-light.png" alt="EcoTrackr dashboard on desktop" width="100%" />
      </picture>
    </td>
    <td>
      <picture>
...
| Категория | Что умеет |
|-----------|-----------|
| **Калькулятор CO₂e** | Транспорт, питание, энергия, покупки, быт — UI и расчёты из npm-пакета [@ecotrackr/co2-calculator](https://github.com/Julie-Trifonova/co2-calculator) |
...
| [@ecotrackr/co2-calculator](https://github.com/Julie-Trifonova/co2-calculator) | Калькулятор CO₂e на дашборде (формы, графики, коэффициенты выбросов) |
...
- `chart-worker.js` — агрегация данных для графиков
- `stats-worker.js` — статистика и KPI
- `leaderboard-worker.js` — ранжирование участников
- `map-worker.js` — обработка гео-точек
...
Калькулятор углеродного следа вынесен в отдельный open-source пакет **[`@ecotrackr/co2-calculator`](https://github.com/Julie-Trifonova/co2-calculator)** — его можно использовать и в других React-проектах.
...
- **Репозиторий:** [github.com/Julie-Trifonova/co2-calculator](https://github.com/Julie-Trifonova/co2-calculator)
...
Язык хранится в `localStorage` (`i18nextLng`). Middleware **редиректит** `/en/*` и `/ru/*` на пути без префикса — чистые URL и проще SEO.
...
Лидерборд, экспорт, статистика — в отдельных workers через singleton `WorkersManager`. Main thread остаётся отзывчивым.
...
UI калькулятора и расчёты CO₂e — в пакете [@ecotrackr/co2-calculator](https://github.com/Julie-Trifonova/co2-calculator). Вспомогательные функции `getEmissionFactor()` / `calculateCO2()` в `lib/utils.ts` используются в других частях приложения (workers, тесты). Не дублируйте коэффициенты в компонентах.
...
│   ├── dashboard/                # FootprintCard, AutoGenerationWidget
...
```bash
git clone https://github.com/Julie-Trifonova/ecotrackr.git
cd ecotrackr
cp .env.example .env.local
...
Copyright © 2026 [Julia Trifonova](https://github.com/Julie-Trifonova)
